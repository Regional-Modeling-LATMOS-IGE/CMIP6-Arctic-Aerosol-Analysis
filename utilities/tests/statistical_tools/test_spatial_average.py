...
from utilities.tools_for_analysis.statistical_tools.spatial_average import (
    adapt_for_spatial_avgd, # to adapt the dataset attributes for regridding
...
#########################################
### TESTS FOR EADAPT_FOR_SPATIAL_AVGD ###
#########################################
...
test2 = adapt_for_spatial_avgd(test2)

def test_good_result_average():
    assert spatial_average_given_field(field = "test", dataset = test2) == np.round(np.mean(test_map),2)