...
def test_not_adapted_for_spatial_average_given_field():
    with pytest.raises(KeyError):
        ...
        spatial_average_given_field(field="test", dataset=test2)


test3 = adapt_for_spatial_avgd(
    test2
)  # need another name here because pytest defines everything before running tests


def test_good_result_average_spatial_average_given_field():
    assert spatial_average_given_field(field="test", dataset=test3) == np.round(
        np.mean(test_map), 2
    )
